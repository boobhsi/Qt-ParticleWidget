...
namespace aaAaa{
    ...
    struct aaSpline{
        ...
        aaSpline(void)
            : type(SPLINE_CUBIC), bLimited(true), limit_left(0.0f),
            limit_top(255.0f), limit_right(255.0f), limit_bottom(0.0f){
            addKnots(aaAaa::aaPoint(0, 0));
            addKnots(aaAaa::aaPoint(128, 150));
            addKnots(aaAaa::aaPoint(255, 255));
        }

        aaSpline(const std::string &spline_name)
            : type(SPLINE_CUBIC), name(spline_name), bLimited(true), limit_left(0.0f),
              limit_top(255.0f), limit_right(255.0f), limit_bottom(0.0f){
            addKnots(aaAaa::aaPoint(0, 0));
            addKnots(aaAaa::aaPoint(128, 150));
            addKnots(aaAaa::aaPoint(255, 255));
    ...
    protected:
        ...
        bool mLimit;
        float mLimit_hor, mLimit_ver;
        float mLimit_top, mLimit_bottom;
        float mLimit_left, mLimit_right;
 ...
 //       aaCurve(const std::string &name, int knum, double *t, double *knot);
        ...
        void getValueList(std::vector<std::pair<double, double>> &list, int sampleCount, bool normalized);
