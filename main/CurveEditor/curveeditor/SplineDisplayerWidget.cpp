...
*/
	...
	void SplineDisplayerWidget::drawSpline(void){
        ...
        std::vector<std::pair<double, double>> valueList;
        pspline->getValueList(valueList, 5000, false); //sample point = 5000 > 255 / 0.1 = 2550
...
*/
        for(int i=0;i<valueList.size();i++) {
            glVertex3f(valueList[i].first, valueList[i].second * Y_FACTOR, Z_VALUE);
        }
	...
	void SplineDisplayerWidget::mouseMoveEvent(QMouseEvent *event)
	{
		...
		if (ctrlSelected >= 0 &&
			event->buttons() & Qt::LeftButton && m_spline_data){
                ...
                if(ctrlSelected != 0 && ctrlSelected != ctrlpoints.size() - 1) ctrlpoints[ctrlSelected].t = npos.t;
